...
import chess.experiments.ex2.SimpleSearcher;
...
public class ParallelSearcher<M extends Move<M>, B extends Board<M, B>> extends
	...
	public M getBestMove(B board, int myTime, int opTime) {
		...
		SimpleTimer.start();
		M move = POOL.invoke(new GetBestMoveTask(board, moves, cutoff, 
	...
	class GetBestMoveTask extends RecursiveTask<BestMove<M>> {
    	...
    	public BestMove<M> compute() {
			...
			if (depth <= cutoff || moves.isEmpty()) {
				BestMove<M> retMove = SimpleSearcher.minimax(evaluator, board, depth);
				return retMove;
			}
